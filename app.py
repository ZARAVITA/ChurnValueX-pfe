...
uploaded_df = None
sample_path = "data//E_Commerce_Dataset.xlsx"  # mettre le fichier d'exemple dans /data
if data_choice == "Télécharger dataset exemple":
    st.sidebar.markdown("Utilisez le dataset d'exemple stocké dans `/data`. (Placez `E_Commerce_Dataset.xlsx` dans /data).")
    use_sample = st.sidebar.button("Charger dataset exemple")
    if use_sample:
        try:
            uploaded_df = pd.read_excel(sample_path, sheet_name=1)
            st.success("Dataset exemple chargé.")
